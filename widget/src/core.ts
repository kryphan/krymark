...
export async function bootKrymark(KEY: string, ORIGIN: string, langHint?: string): Promise<void> {
  ...
  // KHÔNG chạy widget trên BẤT KỲ trang KryMark nào (mọi instance) — tránh chèn đè dashboard,
  // vòng lặp tự-review. Nhận diện qua meta[krymark-app] (layout gắn) HOẶC origin trùng instance.
  if (document.querySelector('meta[name="krymark-app"]')) return;
  try { if (location.origin === new URL(ORIGIN).origin) return; } catch { /* */ }
...
.toast .again{margin-left:10px;background:#fff;color:#1c1c1e;border:0;border-radius:999px;padding:4px 12px;font-size:12px;font-weight:600;cursor:pointer}`;
  ...
  function openSheet() {
    ...
    sheet?.remove();
    sheet = el("div", "sheet");
    // mobile: bottom-sheet cố định. Desktop: vị trí tính SAU khi append (đo kích thước thật + clamp).
    if (innerWidth < 480) sheet.classList.add("bottom");
    sheet.innerHTML = `
    ...
    // Định vị desktop SAU append: đo kích thước thật rồi GHIM gọn trong viewport
    // (fix sheet bị đẩy lệch góc/khuất khi element nằm cao/thấp bất thường).
    if (innerWidth >= 480) {
      const r = (selected as HTMLElement).getBoundingClientRect();
      const sw = sheet.offsetWidth || 340;
      const sh = sheet.offsetHeight || 320;
      const left = Math.min(Math.max(12, r.left), innerWidth - sw - 12);
      let top = r.bottom + 10;
      if (top + sh > innerHeight - 12) top = r.top - sh - 10; // không đủ chỗ dưới → đặt trên element
      top = Math.min(Math.max(12, top), innerHeight - sh - 12); // clamp cứng vào màn hình
  ...
  // Hook cho extension: gọi từ ngoài để vào ngay chế độ chọn (phím tắt Alt+C).
  (window as any).__km_pick = () => { if (mode === "idle") enterPick(); };
  if ((window as any).__km_cfg?.autopick) (window as any).__km_pick();
}
