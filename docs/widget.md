...
2. **Browser extension** (`extension/` in the repo, Manifest V3) — beats CSP because the widget runs in the extension's isolated world. Install: `chrome://extensions` → Developer mode → *Load unpacked* → pick the `extension/` folder. Then the simplest flow: copy the project's **Extension link** (`<origin>/r/<widget_key>`, shown on the project's Settings) → paste it into the extension popup → open any page → **Start review**. No API key needed. (Advanced: connect an API key in the extension's options to browse all projects in a dropdown instead of pasting links.) Screenshots work too (the capture library is bundled).
